...
A condição if (token && token.startsWith('Bearer ')) verifica se o token existe e tem o prefixo "Bearer ". Se sim, remove este prefixo para obter apenas o token.
...
A condição if (token && !isPublicRoute && !isDashboardRoute) verifica se o token existe e se a rota não é pública nem do dashboard. Neste caso, o token é verificado utilizando jwt.verify, passando o token, a chave secreta e uma função de callback para tratar o resultado da verificação.
